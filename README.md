...
- **The web app** (`web/`, hosted on Netlify, data in Supabase) is open in Chrome on each computer and bridges them:
  ...
  - **Chronicle**: Overview (a live feed of the game as it happens, playtime, gold, level and quest charts, status, recent milestones), Characters (each one's journey with a class-coloured hero and XP ring; a character can be deleted), Quests (**completion by zone**: every quest in Classic from the quest database, zone by zone and by class, profession and event, with the percentage each character has done; every quest with where you stand on it; and every quest you logged with the text as you read it), Lore (every text and line of dialogue, filterable by zone for scripts)
  - **World**: Zones (every quest of the zone for the chosen character: ready, later, done, other faction or class; quest givers not met yet and rare spawns as map layers; zones you have not been to; a loose-ends checklist), Locations (zone maps with layers you choose once and keep, creature-density and time-spent heat, nearest services on click, discovery percentage per zone, your own map images, GeoJSON export; every Classic zone map, visited or not), Bestiary (an encyclopedia by kind: beasts, humanoids, undead, critters and the rest, each creature "seen" until killed, with drop rates and sighting maps), People (everyone, each "seen" until met: a quest, a purchase, training, a flight or a talk), Items (a completion journal by type: obtained, seen, and every item in Classic you have yet to find, with percentages), Gathering (herbs, ore, chests and anything else you open)
