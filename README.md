...
- **The web app** (`web/`, hosted on Netlify, data in Supabase) is open in Chrome on each computer and bridges them:
  ...
  - **Chronicle**: Overview (a live feed of the game as it happens, playtime, gold, level and quest charts, status, recent milestones), Characters (each one's page: a Journal that tells the journey as a story from the log, exportable as Markdown; Progress with the class-coloured XP ring, gear, talents, stats, quests and recordings; Places with the zones visited, maps explored and loose ends; a character can be deleted), Lore (the story so far across every character: the storylines started, the lands walked, factions and peoples, the hunt, notable finds), Texts (books and plaques, what people said, what was overheard)
