...
- **The web app** (`web/`, hosted on Netlify, data in Supabase) is open in Chrome on each computer and bridges them:
  ...
  - **World**, the game's own database with your completion marked on it, never per-character counts: Storylines (every quest chain as chapters in order, across zones, with a Markdown outline for writing an episode), Locations (an RPG-style menu: continents → zones by level, with the quests done and the areas discovered in each → the zone's card), Bestiary (continents → zones → every creature there, "not yet" or "killed"; plus an encyclopedia by kind, critters included), People (continents → zones → everyone who gives quests, sells, trains, flies or talks, "not yet" or "met"), Items (kinds → types → every item in the game, obtained or not, with percentages; quest items last), Zones (what happened where you have been, with a loose-ends checklist), Gathering (herbs, ore, chests and anything else you open)
